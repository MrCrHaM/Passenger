function [Adj, s, yy] = genCounty(xval, nSamp)
...
for i = 1:n
    ...
    cons(i).idx = i/n;
    fLat(i) = mean(cons(i).Lat(1:end-1));
    fLon(i) = mean(cons(i).Lon(1:end-1));
    for j = i+1:n
        Adj(i,j) = double(any(ismembertol_custom(Lats{i},Lats{j},1e-10) & ismembertol_custom(Lons{i},Lons{j},1e-10)));
    end
end
